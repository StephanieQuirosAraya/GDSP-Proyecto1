...
AS
SELECT prod.`Name` ProductName, prod.`Description`, prod.Price, PictureID, 
	   charact.`Name` Caracteristica, MaxSelection, charact.CharacteristicID
FROM Products prod
...
INNER JOIN Characteristics charact ON charact.CharacteristicID = characXprod.CharacteristicID;
 ...
 -- SELECT * from ProdNCharact order by ProductName;